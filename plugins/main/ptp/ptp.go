...
func getIntfIP(ifName string, family int) (net.IP, error) {
	sourceIntf, err := netlinksafe.LinkByName(ifName)
	if err != nil {
		return nil, fmt.Errorf("failed to lookup %q: %v", ifName, err)
	}

	addrList, err := netlinksafe.AddrList(sourceIntf, family)
...
func replaceRouteSrcIP(linkIndex int, srcIP net.IP, routes []*types.Route) error {
	...
	for _, r := range routes {
		...
		routeList, err := netlinksafe.RouteListFiltered(family, filter, netlink.RT_FILTER_DST)
		if err != nil {
...
func setupHostVeth(netns string, vethName string, result *current.Result) error {
	...
	if netns != "" {
		...
		err = destNetns.Do(func(_ ns.NetNS) error {
	...
	return destNetns.Do(func(_ ns.NetNS) error {
